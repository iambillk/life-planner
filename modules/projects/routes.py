# modules/projects/routes.py - Complete file with vault integration
import os
...
from .constants import PROJECT_CATEGORIES, PROJECT_STATUSES, PRIORITY_LEVELS, TASK_CATEGORIES
...
def tch_detail(id):
    ...
    # GET VAULT DOCUMENTS INSTEAD OF OLD FILES
    vault_documents = get_entity_documents('project', id)
    
    return render_template('tch_project_detail.html', 
                         project=project,
                         tasks_by_category=tasks_by_category,
                         project_todos=project_todos,
                         vault_documents=vault_documents,  # Pass vault docs instead of files
                         active='tch',
...
def edit_tch(id):
    ...
    if request.method == 'POST':
        ...
        # NEW THIS SECTION: Handle file uploads (same pattern as add_tch)
        uploaded_files = []
        if 'attachments' in request.files:
            files = request.files.getlist('attachments')
...
@projects_bp.route('/tch/<int:id>/vault/upload', methods=['POST'])
def upload_tch_vault_document(id):
    """Upload a document for a TCH project to vault"""
    project = TCHProject.query.get_or_404(id)
    ...
    # Auto-generate tags
    tags = []
    tags.append(f"project_{project.id}")
    tags.append("tch")
    if project.category:
        tags.append(project.category.lower())
    
    try:
        # Use the vault service function
...
# ==================== FILE MANAGEMENT (OLD SYSTEM - KEEPING FOR NOW) ====================
...
def toggle_task(task_id):
    ...
    progress = int((completed_tasks / len(project.tasks)) * 100) if project.tasks else 0
    
    return jsonify({'completed': task.completed, 'progress': progress})

@projects_bp.route('/tch/task/<int:task_id>/edit', methods=['GET', 'POST'])
def edit_tch_task(task_id):
    """Edit a TCH project task"""
    task = TCHTask.query.get_or_404(task_id)
    project = TCHProject.query.get_or_404(task.project_id)
...
def edit_tch_note(note_id):
    ...
    return render_template('tch_edit_note.html', 
                         note=note, 
                         project=project,
                         categories=['general', 'technical', 'business', 'personal'])
...
def personal_index():
    ...
    projects = PersonalProject.query.filter_by(archived=False).order_by(PersonalProject.created_at.desc()).all()
    return render_template('personal_projects.html', projects=projects, active='personal')

@projects_bp.route('/personal/add', methods=['GET', 'POST'])
def add_personal():
    ...
    if request.method == 'POST':
        ...
        db.session.add(project)
        db.session.commit()
        flash(f'Personal project "{project.name}" created!', 'success')
        return redirect(url_for('projects.personal_detail', id=project.id))
    
    return render_template('personal_add_project.html', active='personal')

@projects_bp.route('/personal/<int:id>')
def personal_detail(id):
    """View personal project details"""
    project = PersonalProject.query.get_or_404(id)
    return render_template('personal_project_detail.html', project=project, active='personal')

@projects_bp.route('/personal/<int:id>/edit', methods=['GET', 'POST'])
def edit_personal(id):
    ...
    if request.method == 'POST':
        ...
        project.updated_at = datetime.utcnow()
        db.session.commit()
        flash('Personal project updated!', 'success')
        return redirect(url_for('projects.personal_detail', id=id))
    
    return render_template('personal_edit_project.html', project=project, active='personal')

@projects_bp.route('/personal/<int:id>/delete', methods=['POST'])
def delete_personal(id):
    ...
    db.session.delete(project)
    db.session.commit()
    flash(f'Personal project "{name}" deleted!', 'success')
