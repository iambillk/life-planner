...
def _to_text(x):
    """Return first non-empty string from possibly nested lists/tuples/values."""
    if x is None:
        return None
...
def parse_vcf(file_bytes: bytes) -> Tuple[List[ImportRow], List[str]]:
    ...
    for block in blocks:
        ...
        # Title & Company (ORG can be nested lists; normalize safely)
        title = ""
        comp = None
        if getattr(v, "title", None):
            title = ( _to_text(v.title.value) or "" ).strip()
        if getattr(v, "org", None):
            comp = ( _to_text(v.org.value) or "" ).strip()

        # Website / URL can also be odd shapes occasionally
        url = None
        if getattr(v, "url", None):
            url = ( _to_text(v.url.value) or "" ).strip()
